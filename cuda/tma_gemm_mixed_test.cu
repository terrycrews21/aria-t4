// Mini-GEMM MIXTE (correctness) — chargement mixte d'alpha : A=cp.async, B=TMA, dans le même GEMM.
// Single-tile : 1 CTA calcule C(128×256) = A(128×128)·B(256×128)^T (TN, int8→int32).
// A : cp.async (SM80_CP_ASYNC, coords alpha recette §25) → smem swizzlé alpha Swizzle<3,4,3>, ldmatrix ×4.
// B : SM90_TMA_LOAD → smem swizzlé alpha Swizzle<3,4,3>, ldmatrix ×2. MMA 8 warps (atom 2×4).
// Synchro HYBRIDE : cp_async_wait<0> (A) + wait_barrier (B). But : C == réf GEMM int8 bit-exact.
// = valide le maillon technique NEUF de la greffe #1 AVANT portage dans le grind. Recette = ALPHA_NATIVE_RECIPE.md.
...
template <class CopyA, class TmaB, class GA, class GB, class SA, class SB, class Mma,
          class S2RA, class S2RB>
__global__ static __launch_bounds__(256)
void mixed_gemm(int8_t const* A, int8_t const* B, int32_t* C, int M, int N, int K,
              CopyA copy_a, CUTE_GRID_CONSTANT TmaB const tma_b,
  ...
  // ---- A : cp.async (recette §21 : A=cp.async), single tile ----
  Tensor mA = make_tensor(make_gmem_ptr(A), gA_layout);   // (128,128)
  ...
  // ---- B : TMA (single tile), pattern prouvé (variables nommées _x) ----
  ...
  if (threadIdx.x == 0) {
    ...
    cute::set_barrier_transaction_bytes(smem.mbar[0], int(256*128*sizeof(int8_t)));
    copy(tma_b.with(smem.mbar[0]), tBgB(_,0), tBsB(_,0));
  }
  // ---- synchro HYBRIDE : A (cp.async) + B (TMA) ----
  cp_async_wait<0>();
  __syncthreads();
...
int main() {
  ...
  // A : layout STATIQUE (cp.async exige une stride contiguë statique Int<1> pour vectoriser en uint128)
  auto gA_layout = make_layout(make_shape(Int<128>{},Int<128>{}), make_stride(Int<128>{}, Int<1>{}));
  ...
  // TiledCopy cp.async pour A (recette §25 : 256 threads, A 128×128) : atom uint128, TV ((8,32),16), tiler 32×128
  ...
  TiledCopy<AtomG, TVcopy, TilerC> copy_a;
  ...
  auto* k = &mixed_gemm<decltype(copy_a),decltype(tma_b),decltype(gA_layout),decltype(gB_layout),
  ...
  k<<<1,256,smem_size>>>(dA,dB,dC,M,N,K,copy_a,tma_b,gA_layout,gB_layout,sA_layout,sB_layout,mma,s2r_a,s2r_b);
  ...
  if(mism==0) printf("MINI-GEMM MIXTE OK — C bit-exact vs ref (128x256x128 int8, A=cp.async + B=TMA, 8 warps)\n");
