...
#include "pearl_gpu_kernel_sm75_wideK.cuh" // variant K: wide3 + ldmatrix fragment loads (~589ms, +6%)
...
static int resident_run(Ctx* c, uint64_t setup_seed,
  ...
  if ((dev_major == 7 || getenv("ARIA_FORCE_SM75")) && getenv("ARIA_T4_WIDEK")) {
    // Variant K: wide3 + ldmatrix fragment loads. Measured 589.48/591.41ms vs
    // wide3 625.74/631.48ms on T4 (found counts + hit-set signature identical
    // across 13 setups at ~25% bound -> transcript bit-exact). Same geometry
    // and proof output as wide3/wide: one full 16x16 tile per hit.
    if ((m % aria_sm75_wideK::kBlockM) != 0 || (n % aria_sm75_wideK::kBlockN) != 0) {
      fprintf(stderr, "sm75 wideK kernel requires m%%128==0 and n%%256==0 (m=%d n=%d)\n", m, n);
      return -1;
    }
    dim3 gridWK((unsigned)(m / aria_sm75_wideK::kBlockM),
                (unsigned)(n / aria_sm75_wideK::kBlockN));
    dim3 blockWK(aria_sm75_wideK::kThreads);
    ARIA_NEXT("gemm launch sm75-wideK seed=%016llx m=%d n=%d k=%d grid=(%u,%u) blk=%u rank=%d",
              (unsigned long long)setup_seed, m, n, k, gridWK.x, gridWK.y,
              blockWK.x, c->rank);
    if (c->big_endian) {
      aria_sm75_wideK::grind<true><<<gridWK, blockWK, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    } else {
      aria_sm75_wideK::grind<false><<<gridWK, blockWK, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    }
  ...
  if ((dev_major == 7 || getenv("ARIA_FORCE_SM75")) && getenv("ARIA_T4_WIDE")) {
    ...
    if ((m % aria_sm75_wide::kBlockM) != 0 || (n % aria_sm75_wide::kBlockN) != 0) {
      fprintf(stderr, "sm75 wide kernel requires m%%128==0 and n%%256==0 (m=%d n=%d)\n", m, n);
      return -1;
    }
    dim3 gridW((unsigned)(m / aria_sm75_wide::kBlockM),
               (unsigned)(n / aria_sm75_wide::kBlockN));
    dim3 blockW(aria_sm75_wide::kThreads);
    ARIA_NEXT("gemm launch sm75-wide seed=%016llx m=%d n=%d k=%d grid=(%u,%u) blk=%u rank=%d",
              (unsigned long long)setup_seed, m, n, k, gridW.x, gridW.y,
              blockW.x, c->rank);
    if (c->big_endian) {
      aria_sm75_wide::grind<true><<<gridW, blockW, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    } else {
      aria_sm75_wide::grind<false><<<gridW, blockW, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    }
    if (c->do_timing) cudaEventRecord(c->tEnd, c->sA);
  ...
  if ((dev_major == 7 || getenv("ARIA_FORCE_SM75")) && getenv("ARIA_T4_DUAL")) {
    ...
    // performs each fold in registers + warp shuffles and reuses A across a
    // 64x256 CTA. Peak/default dimensions are exact multiples of both axes.
    if ((m % aria_sm75_dual::kBlockM) != 0 || (n % aria_sm75_dual::kBlockN) != 0) {
    ...
    ARIA_NEXT("gemm launch sm75-dual seed=%016llx m=%d n=%d k=%d grid=(%u,%u) blk=%u rank=%d",
              ...
              block75.x, c->rank);
    if (c->big_endian) {
      aria_sm75_dual::grind<true><<<grid75, block75, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    } else {
      aria_sm75_dual::grind<false><<<grid75, block75, 0, c->sA>>>(
          c->d_A, c->d_B, m, n, k, c->rank, c->d_as, c->d_bnd,
          c->d_found, c->d_hr, c->d_hc, max_hits);
    }
    if (c->do_timing) cudaEventRecord(c->tEnd, c->sA);
...
extern "C" {
int pearl_gpu_device_major() { return aria_device_major(); }
