// Standalone perf + coords bench for the aria_sm75_wide2::grind kernel.
...
#include "pearl_gpu_kernel_sm75_wide2.cuh"
...
int main(int argc, char** argv) {
  ...
  printf("sm75_wide2_bench m=%d n=%d k=%d iters=%d rank=%d\n", M, N, K, iters, rank);
  if (M % aria_sm75_wide2::kBlockM != 0 || N % aria_sm75_wide2::kBlockN != 0) {
    printf("bad dims: need M%%%d==0 N%%%d==0\n", aria_sm75_wide2::kBlockM, aria_sm75_wide2::kBlockN);
    return 2;
  }
  ...
  CK(cudaMemset(dbnd, 0xFF, 32));  // every transcript hashes below bound -> every warp tile hits

  dim3 grd((unsigned)(M / aria_sm75_wide2::kBlockM), (unsigned)(N / aria_sm75_wide2::kBlockN));
  dim3 blk(aria_sm75_wide2::kThreads);
  printf("grid=(%u,%u) block=%u\n", grd.x, grd.y, blk.x);

  // Warmup (clock ramp + caches)
  for (int i = 0; i < 2; ++i) {
    CK(cudaMemset(dfound, 0, 4));
    aria_sm75_wide2::grind<false><<<grd, blk, 0>>>(dA, dB, M, N, K, rank, dkey, dbnd, dfound, dhr, dhc, MAXH);
  }
