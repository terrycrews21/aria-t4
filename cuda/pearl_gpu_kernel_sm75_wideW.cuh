// ariaminer — Turing sm_75 variant W: 128x128 CTA, 512 thr, 2 CTAs/SM.
// ncu on wideK: latency-bound, 4 warps/scheduler, 67% no-eligible cycles.
// All 2-CTA attempts so far failed: wideL (8 tiles/warp = spill), wideS/T
// (64x128 = B DRAM traffic doubled -> 949ms). wideW halves ONLY N per CTA:
// warp owns 32x32 = 2x2 = 4 tiles (acc 32 regs), 4x4 warp grid = 16 warps.
// smem: 2 stages x (128+128 rows)*12 words*4B = 24.6KB + transcripts 4KB
//   = 28.6KB <= 32KB budget -> 2 CTAs/SM = 32 warps/SM = 8 warps/scheduler.
// Grid (128, 512): total B DRAM traffic unchanged vs wideK (each B strip
// still served to 128 M-blocks); A traffic per B-strip halved.
// ldmatrix: x4 for the 4 A frags + x4 for the 4 B frags per substep.
//
...
void grind(const int8_t* __restrict__ a,
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    ...
    for (int sub = 0; sub < kChunkK / kMmaK; ++sub) {
      ...
      uint32_t bv[kTileColsPerWarp * 2];     // 4 8x16 B fragments (32 cols)
      {
