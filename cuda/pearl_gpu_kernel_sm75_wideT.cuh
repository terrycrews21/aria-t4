// ariaminer — Turing sm_75 variant T: wideS + 3-stage pipeline (2 CTAs/SM).
// smem: 3 stages x 9.2KB + 2KB transcripts = ~30KB/CTA -> 2 CTAs fit 64KB.
//
...
void grind(const int8_t* __restrict__ a,
  ...
  __shared__ __align__(16) uint32_t stages[kStages][kWordsStage];
  ...
  int pf_slot = 0;  // alternates 0/1 across prefetches
  auto prefetch = [&](int k_offset) {
    if (has_a) {
      int row = tid / 2;
      int col_vec = tid % 2;
      prefetched_a[pf_slot] = __ldcg(reinterpret_cast<const uint4*>(
          a + static_cast<size_t>(row_base + row) * k + k_offset + col_vec * 16));
    }
    if (tid < kBlockN * 2) {
      int cidx = tid / 2;
      int col_vec = tid % 2;
      prefetched_b[pf_slot] = __ldcg(reinterpret_cast<const uint4*>(
          bt + static_cast<size_t>(col_base + cidx) * k + k_offset + col_vec * 16));
    }
  };

  auto publish = [&](int stage) {
    // consume the slot the most recent prefetch wrote, then flip for next round
    if (has_a) {
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    ...
    const int next_chunk = chunk + 2;   // stage written 2 steps ago is consumed now
    const bool have_next = next_chunk < chunks;
    if (have_next) prefetch(next_chunk * kChunkK);
