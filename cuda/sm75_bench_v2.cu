// Bench: wide3 vs 128x128 (2 blocks/SM)
#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <vector>
...
#include "pearl_gpu_kernel_sm75_128x128.cuh"
...
int main(int argc, char** argv) {
  ...
  // wide3: 128x256, 512 threads, 1 block/SM
  {
    dim3 grd(M/128, N/256), blk(aria_sm75_wide3::kThreads);
    ...
    printf("wide3  128x256 512thr 1blk: %7.2f ms  %6.2f TH/s int  %6.2f disp\n", per, ths, ths*1.1);
  }

  // 128x128: 256 threads, 2 blocks/SM
  {
    dim3 grd(M/128, N/128), blk(aria_sm75_128x128::kThreads);
    aria_sm75_128x128::grind<false><<<grd, blk>>>(dA,dB,M,N,K,rank,dkey,dbnd,dfound,dhr,dhc,64);
    CK(cudaDeviceSynchronize());
    cudaEvent_t t0,t1; CK(cudaEventCreate(&t0)); CK(cudaEventCreate(&t1));
    CK(cudaEventRecord(t0));
    for (int i=0;i<iters;i++) {
    ...
    float ms; CK(cudaEventElapsedTime(&ms,t0,t1));
    double per=ms/iters, ths=work/(per/1000.0)/1e12;
    printf("128x128 256thr 2blk:      %7.2f ms  %6.2f TH/s int  %6.2f disp\n", per, ths, ths*1.1);
  }
