// ariaminer — Turing sm_75 variant: 64x128 tile, 256 threads, 2 blocks/SM
#pragma once
...
static constexpr int kWarpRows = 2;
static constexpr int kWarpCols = 4;
static constexpr int kTileRowsPerWarp = 2;   // 2*16 = 32 rows... wait 2 warps * 32 = 64
static constexpr int kTileColsPerWarp = 2;   // 2*16 = 32 cols... 4 warps * 32 = 128
...
static constexpr int kWordsStage = kWordsA + kWordsB;  // 2304
...
void grind(const int8_t* __restrict__ a,
  ...
  uint4 prefetched_a;
  uint4 prefetched_b;
  // A: 64 rows x 2 uint4 = 128 loads; B: 128 rows x 2 uint4 = 256 loads
  const bool has_a = tid < (kBlockM * 2);   // 128 threads load A
