// Bench: wide3 vs wide6 (3-stage) vs wide9 (chunkK=64)
#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <vector>
...
#include "pearl_gpu_kernel_sm75_wide9.cuh"
...
int main(int argc, char** argv) {
  ...
  printf("bench_all: m=%d n=%d k=%d iters=%d rank=%d\n", M, N, K, iters, rank);
  ...
  // wide3 baseline
  {
    ...
    printf("wide3  2st k32 s12 static: %7.2f ms  %6.2f TH/s int  %6.2f disp\n", per, ths, ths*1.1);
  }

  // wide6: 3-stage dynamic smem, transcripts in dynamic smem
  {
    dim3 blk(aria_sm75_wide6::kThreads);
    size_t smem = aria_sm75_wide6::kDynamicSmemBytes;
    printf("wide6 smem bytes: %zu\n", smem);
    cudaError_t attr_err = cudaFuncSetAttribute((const void*)aria_sm75_wide6::grind<false>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem);
    if (attr_err != cudaSuccess) {
      printf("wide6 attr failed: %s\n", cudaGetErrorString(attr_err));
    } else {
      cudaFuncSetAttribute((const void*)aria_sm75_wide6::grind<false>,
      ...
      if (err != cudaSuccess) {
        ...
        float ms; CK(cudaEventElapsedTime(&ms,t0,t1));
        double per=ms/iters, ths=work/(per/1000.0)/1e12;
        printf("wide6  3st k32 s12 dyn  : %7.2f ms  %6.2f TH/s int  %6.2f disp\n", per, ths, ths*1.1);
      }
    }
  }

  // wide9: chunkK=64, stride 20, transcripts global
  {
    dim3 blk(aria_sm75_wide9::kThreads);
    size_t smem = aria_sm75_wide9::kDynamicSmemBytes;
    printf("wide9 smem bytes: %zu\n", smem);
    cudaError_t attr_err = cudaFuncSetAttribute((const void*)aria_sm75_wide9::grind<false>,
        cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem);
    if (attr_err != cudaSuccess) {
      printf("wide9 attr failed: %s\n", cudaGetErrorString(attr_err));
    } else {
      cudaFuncSetAttribute((const void*)aria_sm75_wide9::grind<false>,
          cudaFuncAttributePreferredSharedMemoryCarveout, 100);
      uint32_t* d_transcript;
      int grid_size = (M/128) * (N/256);
      size_t trans_size = (size_t)grid_size * aria_sm75_wide9::kWarps * aria_sm75_wide9::kTilesPerWarp * aria_sm75_wide9::kTranscript * sizeof(uint32_t);
      CK(cudaMalloc(&d_transcript, trans_size));
      aria_sm75_wide9::grind<false><<<grd, blk, smem>>>(dA,dB,M,N,K,rank,dkey,dbnd,dfound,dhr,dhc,64,d_transcript);
      cudaError_t err = cudaDeviceSynchronize();
      if (err != cudaSuccess) {
        printf("wide9 FAILED: %s\n", cudaGetErrorString(err));
      } else {
        cudaEvent_t t0,t1; CK(cudaEventCreate(&t0)); CK(cudaEventCreate(&t1));
        CK(cudaEventRecord(t0));
        for (int i=0;i<iters;i++) {
        ...
        float ms; CK(cudaEventElapsedTime(&ms,t0,t1));
        double per=ms/iters, ths=work/(per/1000.0)/1e12;
        printf("wide9  2st k64 s20 dyn  : %7.2f ms  %6.2f TH/s int  %6.2f disp\n", per, ths, ths*1.1);
      }
    }
  }
