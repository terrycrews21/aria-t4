// ariaminer — Turing sm_75 variant Q: wideK + fragment double-buffering.
// ncu on wideK: scheduler 67% no-eligible cycles -> warps stall waiting on
// ldmatrix results before issuing mma. Double-buffer the fragments: issue
// ldmatrix for substep s+1 while mma of substep s executes (+12 regs).
//
...
void grind(const int8_t* __restrict__ a,
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    ...
    // Fragment double-buffer: load sub 0, then per sub issue loads of sub+1
    // before mma of sub.
    uint32_t avA[kTileRowsPerWarp * 2], bvA[kTileColsPerWarp * 2];
    uint32_t avB[kTileRowsPerWarp * 2], bvB[kTileColsPerWarp * 2];
    {
      const int k_word = 0;
      const uint32_t addr_a = __cvta_generic_to_shared(
          sa + (a_row_lo + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
      ldmatrix_x4(avA[0], avA[1], avA[2], avA[3], addr_a);
      const uint32_t addr_b0 = __cvta_generic_to_shared(
          sb + (b_col_lo + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
      ldmatrix_x4(bvA[0], bvA[1], bvA[2], bvA[3], addr_b0);
      const uint32_t addr_b1 = __cvta_generic_to_shared(
          sb + (b_col_lo + 32 + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
      ldmatrix_x4(bvA[4], bvA[5], bvA[6], bvA[7], addr_b1);
    }
    #pragma unroll
    for (int sub = 0; sub < kChunkK / kMmaK; ++sub) {
      uint32_t* av_cur = (sub & 1) ? avB : avA;
      uint32_t* bv_cur = (sub & 1) ? bvB : bvA;
      uint32_t* av_nxt = (sub & 1) ? avA : avB;
      uint32_t* bv_nxt = (sub & 1) ? bvA : bvB;
      if (sub + 1 < kChunkK / kMmaK) {
        const int k_word = (sub + 1) * (kMmaK / 4);
        const uint32_t addr_a = __cvta_generic_to_shared(
            sa + (a_row_lo + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
        ldmatrix_x4(av_nxt[0], av_nxt[1], av_nxt[2], av_nxt[3], addr_a);
        const uint32_t addr_b0 = __cvta_generic_to_shared(
            sb + (b_col_lo + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
        ldmatrix_x4(bv_nxt[0], bv_nxt[1], bv_nxt[2], bv_nxt[3], addr_b0);
        const uint32_t addr_b1 = __cvta_generic_to_shared(
            sb + (b_col_lo + 32 + ldm_mat * 8 + ldm_row) * kRowStride + k_word);
        ldmatrix_x4(bv_nxt[4], bv_nxt[5], bv_nxt[6], bv_nxt[7], addr_b1);
      }
      #pragma unroll
      for (int tm = 0; tm < kTileRowsPerWarp; ++tm) {
        ...
        for (int tn = 0; tn < kTileColsPerWarp; ++tn) {
          ...
          mma_8x8x16(acc[t][0], acc[t][1], av_cur[2 * tm],     bv_cur[2 * tn]);
          mma_8x8x16(acc[t][2], acc[t][3], av_cur[2 * tm + 1], bv_cur[2 * tn]);
          mma_8x8x16(acc[t][4], acc[t][5], av_cur[2 * tm],     bv_cur[2 * tn + 1]);
          mma_8x8x16(acc[t][6], acc[t][7], av_cur[2 * tm + 1], bv_cur[2 * tn + 1]);
        }
      }
    }
