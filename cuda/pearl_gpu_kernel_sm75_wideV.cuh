// ariaminer — Turing sm_75 variant V: wideK + 3-stage pipeline.
// smem budget: 3 stages x 18.4KB + 8KB transcripts = 63.3KB <= 64KB sm_75.
// Deeper pipeline hides the publish+barrier phase behind 2 in-flight loads.
// Two alternating prefetch register slots (prefetch chunk+2 while chunk+1 is
// staged and chunk computes).
//
...
// Dynamic smem = stages ONLY (transcripts stay static 8KB).
// Total per block = 3x18432 + 8192 = 63488B <= 64KB sm_75 cap.
static constexpr int kDynamicSmemBytes = kStages * kWordsStage * 4;
...
void grind(const int8_t* __restrict__ a,
  ...
  uint4 prefetched_a[2];
  uint4 prefetched_b[2];
  ...
  auto prefetch = [&](int k_offset) {
    if (has_a) {
      ...
      prefetched_a[pf_slot] = __ldcg(ptr_a);
    }
    {
      ...
      prefetched_b[pf_slot] = __ldcg(ptr_bt);
    }
  };

  auto publish = [&](int stage) {
    // publishes the slot most recently filled by prefetch
    if (has_a) {
      reinterpret_cast<uint4*>(stages[stage])[(tid / 2) * 3 + (tid % 2)] = prefetched_a[pf_slot];
    }
    {
      reinterpret_cast<uint4*>(stages[stage] + kWordsA)[(tid / 2) * 3 + (tid % 2)] = prefetched_b[pf_slot];
    }
    pf_slot ^= 1;
  };
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    ...
    const int ahead = chunk + 2;              // load 2 chunks ahead into free stage
    const bool have_next = ahead < chunks;
    if (have_next) prefetch(ahead * kChunkK);
