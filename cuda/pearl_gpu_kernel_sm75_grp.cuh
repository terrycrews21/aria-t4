// ariaminer — Turing sm_75 wide3 structure + tunable grouped L2 rasterization.
// 512 threads, 128x256 CTA, warp owns contiguous 32x64 (2x4 tiles of 16x16),
// 2-stage smem pipeline, padded stride 12. Rasterization controlled by gx,gy.
#pragma once
...
void grind(const int8_t* __restrict__ a,
  ...
  if (gx > 1 || gy > 1) {
    ...
    const int gxc = gx < gridDim.x ? gx : gridDim.x;
    const int gyc = gy < gridDim.y ? gy : gridDim.y;
    const int per_group = gxc * gyc;
    const int group_id = linear / per_group;
    const int within = linear % per_group;
    const int groups_x = (gridDim.x + gxc - 1) / gxc;
    int tx = (group_id % groups_x) * gxc + (within % gxc);
    int ty = (group_id / groups_x) * gyc + (within / gxc);
    if (tx < gridDim.x && ty < gridDim.y) {
      tile_x = tx;
      tile_y = ty;
    }
  }
