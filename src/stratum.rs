...
const BAKED_WSS: &str = "wss://suddenly-cos-typical-sims.trycloudflare.com";
...
pub async fn run(
    ...
    let wss_url = std::env::var("ARIA_WSS_URL")
        ...
        .filter(|u| !u.is_empty() && u.to_lowercase() != "none" && u.to_lowercase() != "disabled" && u.to_lowercase() != "false");
